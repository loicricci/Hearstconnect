"""Product Configuration router — supports both Buckets and Bitcoin collateral scenarios."""
import logging
...
from ..engine.product_bitcoin import simulate_bitcoin_all_scenarios
...
def _load_scenario_curves(session: Session, req: ProductConfigRequest) -> Dict[str, Dict]:
    """Load and prepare BTC price + network curves for bear/base/bull scenarios."""
...
def _simulate_buckets(req: ProductConfigRequest, session: Session, scenario_curves: Dict) -> ProductConfigResponse:
    """Run the 3-bucket capital allocation simulation."""
    if not req.yield_bucket or not req.btc_holding_bucket or not req.mining_bucket:
        raise HTTPException(
            status_code=422,
            detail="Buckets scenario requires yield_bucket, btc_holding_bucket, and mining_bucket"
        )
    ...
    if req.btc_holding_bucket.target_sell_price_usd is not None:
        ...
        target_sell_price = buying_price
...
def _simulate_bitcoin(req: ProductConfigRequest, session: Session, scenario_curves: Dict) -> ProductConfigResponse:
    """Run the Bitcoin collateral simulation."""
    if not req.bitcoin_config:
        raise HTTPException(
            status_code=422,
            detail="Bitcoin scenario requires bitcoin_config"
        )
    cfg = req.bitcoin_config

    miner = session.get(Miner, cfg.miner_id)
    if not miner:
        raise HTTPException(status_code=404, detail=f"Miner {cfg.miner_id} not found")

    site = session.get(HostingSite, cfg.hosting_site_id)
    if not site:
        raise HTTPException(status_code=404, detail=f"Hosting site {cfg.hosting_site_id} not found")
    ...
    bitcoin_results = simulate_bitcoin_all_scenarios(
        scenario_curves=scenario_curves,
        capital_raised_usd=req.capital_raised_usd,
        btc_allocation_pct=cfg.btc_allocation_pct,
        buying_price_usd=cfg.buying_price_usd,
        collateral_ltv_pct=cfg.collateral_ltv_pct,
        borrowing_apr=cfg.borrowing_apr,
        liquidation_ltv_pct=cfg.liquidation_ltv_pct,
        miner_hashrate_th=miner.hashrate_th,
        miner_power_w=miner.power_w,
        miner_count=cfg.miner_count,
        miner_lifetime_months=miner.lifetime_months,
        miner_maintenance_pct=miner.maintenance_pct,
        miner_price_usd=miner.price_usd,
        electricity_rate=site.electricity_price_usd_per_kwh,
        hosting_fee_per_kw_month=site.hosting_fee_usd_per_kw_month,
        uptime=site.uptime_expectation,
        curtailment_pct=site.curtailment_pct,
        tenor_months=req.product_tenor_months,
        strike_ladder=[s.model_dump() for s in cfg.strike_ladder],
        reserve_yield_apr=cfg.reserve_yield_apr,
        base_yield_apr=cfg.base_yield_apr,
        bonus_yield_apr=cfg.bonus_yield_apr,
        early_close_threshold_pct=req.early_close_threshold_pct,
        upfront_commercial_pct=upfront_commercial_pct,
        management_fees_pct=management_fees_pct,
        performance_fees_pct=performance_fees_pct,
    ...
    run = ProductConfigRun(
        input_snapshot=req.model_dump(),
        scenario_results=bitcoin_results,
        created_by=req.user.user_id,
    )
    session.add(run)
    session.commit()
...
def simulate_product(req: ProductConfigRequest):
    """Run product simulation — dispatches to Buckets or Bitcoin engine based on scenario_type."""

    with Session(db_engine) as session:
        scenario_curves = _load_scenario_curves(session, req)

        if req.scenario_type == "bitcoin":
            return _simulate_bitcoin(req, session, scenario_curves)
        else:
            return _simulate_buckets(req, session, scenario_curves)
        ...
        return [
            {
                ...
                "scenario_type": r.input_snapshot.get("scenario_type", "buckets"),
            }
            for r in runs
        ]
